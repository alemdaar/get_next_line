...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/04 22:48:15 by oelhasso          #+#    #+#             */
/*   Updated: 2025/02/21 17:48:30 by oelhasso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!left[fd])
	{
		left[fd] = ft_strdup_add("", 0);
		if (!left[fd])
			return (NULL);
	}
	if (fd < 0 || BUFFER_SIZE < 0 || read (fd, 0, 0) == -1)
		return (free_str(&left[fd]), NULL);
	buffer = ft_strdup_add(left[fd], BUFFER_SIZE);
	free_str(&left[fd]);
	if (!buffer)
		return (free_str(&left[fd]), NULL);
	rtn = read_it(buffer, fd, &left[fd]);
	return (rtn);
}
